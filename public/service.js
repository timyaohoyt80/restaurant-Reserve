...
//angular module for angular-view
//'ngRoute', 'ngAnimate', 'ui.bootstrap',依赖，related
var webapp = angular.module('reservationApp', ['ngRoute', 'ngAnimate', 'ui.bootstrap']);

//对应template的login.html
//owner login
    ...
    self.login = function () {
        UserService.login(this.user).then(function (res) {
            ...
            (self.open = function () {
                $uibModal.open({
                    animation: true,
                    //对于报错页面，template,loginErrorReminder.html
                    templateUrl: '/templates/loginErrorReminder.html',
...
//对于detail.html模板，负责显示具体login的内容
...
webapp.controller('MakeReservationController', ['$scope','ResService', 'ownerService', '$uibModal',function ($scope, ResService, ownerService, $uibModal) {
    ...
    self.make = function () {
        ResService.makeReserve(self.reservation).then(function (res) {
            ...
            (self.open = function () {
                $uibModal.open({
                    animation: true,
                    //加载reminCode模板
                    templateUrl: '/templates/researveCode.html',
                    controller: 'ModalInstanceCtrl as templateCtrl',
                    resolve: {
                        items: function () {
                            return self.result;
        ...
        //this part for auto assign,这个auto assign功能目前不支持
        if(self.auto){
    ...
    self.find = function () {
        ResService.getReserve(self.id).then(function (res) {
            ...
            (self.open = function () {
                $uibModal.open({
                    animation: true,
                    templateUrl: '/templates/checkRes.html',
...
webapp.config(function ($routeProvider) {
    $routeProvider.when('/', {
        ...
        templateUrl: 'templates/findReservation.html',
        controller: 'findReservationController as findCtrl'
    }).when('/changeRes', {
        templateUrl: 'templates/changeReservation.html',
        controller: 'changeReservationController as changeCtrl'
    }).when('/getTable/:code', {
        templateUrl: 'templates/setTable.html',
